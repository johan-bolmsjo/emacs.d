...
Requires GNU Emacs version 31.1 or greater.
...
This package manager keeps the complete upstream Git repositories available together with lock files to pin down exact versions.
...
- Project like IDE support using speedbar-window, [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Projects.html][project.el]] and [[https://github.com/joaotavora/eglot][Eglot]].
...
- ~C-c j~ :: =avy-goto-symbol-1-above=
- ~C-c k~ :: =avy-goto-symbol-1-below=
- ~C-c l~ :: =avy-goto-line=

**** Window Management

The /winner-mode/ is enabled by default. Restore the window layout with
~C-c <left>~ or redo with ~C-c <right>~. Useful for peeking into buffers
with e.g. ~C-x 1~ and then restoring the window layout.
...
Spell checking is configured to use hunpell with English and Swedish dictionaries.
These must be installed through the host operating system.
The default dictionary is English, switch with ~M-x ispell-change-dictionary~.
...
#+begin_src bash
  # Select toolchain version as appropriate.
  # You also need to update the GOROOT and PATH environment variables for your shell.
...
#+begin_src bash
  # Debugger for Go (versioned as the Go toolchain, e.g. v1.26).
...
#+begin_src bash
  wget https://ftp.acc.umu.se/mirror/gnu.org/gnu/emacs/emacs-31.1.tar.xz
